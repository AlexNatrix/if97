...
func getSubRegionPS(pressure float64, entropy float64) SubRegion {
	switch {
	case pressure > 4:
		if entropy < 5.85 {
			return C
...
func IsentropicExponentPT(pressure float64, temperature float64) float64 {
	...
	pi := pressure / pRef
	tau := Tref / temperature
	gPi := gammaOPi(pi) + gammaRPi(pi, tau)
	gRPiPi := gammaRPiPi(pi, tau)
	x := 1 + pi*gammaRPi(pi, tau)
	y := x * tau * tau * (gammaOTauTau(tau) + gammaRTauTau(pi, tau))
	z := x - pi*tau*gammaRPiTau(pi, tau)

	return x * y / ((1-pi*pi*gRPiPi)*y + pi*gPi*z*z)
}
...
func PressureHS(enthalpy float64, entropy float64) float64 {
	...
	if enthalpy > h2ab {
		if entropy < 5.85 {
			// region 2c
			x = []float64{enthalpy/3500 - 0.7, entropy/5.9 - 1.1, 100}
			IJn = IJn2cHS

		} else {
			// region 2b
			x = []float64{enthalpy/4100 - 0.6, entropy/7.9 - 1.01, 100}
			IJn = IJn2bHS
		}

	} else {
		// region 2a
		x = []float64{enthalpy/4200 - 0.5, entropy/12 - 1.2, 4}
		IJn = IJn2aHS
	}
...
func SpecificEnthalpyPT(pressure float64, temperature float64) float64 {
	...
	return tau * (gammaOTau(tau) + gammaRTau(pressure/pRef, tau)) * constants.R * temperature
}
...
// @Override
func specificEntropyRhoT(rho float64, T float64) float64 {

	panic("Region2.specificEntropyRhoT() pending implementation.Im sorry")
}
...
func SpecificIsobaricHeatCapacityPT(pressure float64, temperature float64) float64 {
	...
	return -tau * tau * (gammaOTauTau(tau) + gammaRTauTau(pressure/pRef, tau)) * constants.R
}
...
func SpecificIsochoricHeatCapacityPT(pressure float64, temperature float64) float64 {
	...
	return (-tau*tau*(gammaOTauTau(tau)+gammaRTauTau(pi, tau)) - x*x/(1-pi*pi*gammaRPiPi(pi, tau))) * constants.R
}
...
func SpecificVolumePT(pressure float64, temperature float64) float64 {
	...
	return pi * (gammaOPi(pi) + gammaRPi(pi, Tref/temperature)) / 1e3 * constants.R * temperature / pressure
}
...
*
  ...
  - @return
*/
func theta2aPH(pi float64, eta float64) float64 {

	var out float64 = 0

	for _, ijn := range IJn2aPH {
		out += ijn[2] * math.Pow(pi, ijn[0]) * math.Pow(eta-2.1, ijn[1])
	}
	return out
}
 ...
 * @return
 */
func theta2aPS(pi float64, sigma float64) float64 {

	var out float64 = 0

	for _, ijn := range IJn2aPS {
		out += ijn[2] * math.Pow(pi, ijn[0]) * math.Pow(sigma-2, ijn[1])
	}
	return out
}
 ...
 * @return
 */
func theta2bPH(pi float64, eta float64) float64 {

	var out float64 = 0

	for _, ijn := range IJn2bPH {
		out += ijn[2] * math.Pow(pi-2, ijn[0]) * math.Pow(eta-2.6, ijn[1])
	}
	return out
}
 ...
 * @return
 */
func theta2bPS(pi float64, sigma float64) float64 {

	var out float64 = 0

	for _, ijn := range IJn2bPS {
		out += ijn[2] * math.Pow(pi, ijn[0]) * math.Pow(10-sigma, ijn[1])
	}
	return out
}
 ...
 * @return
 */
func theta2cPH(pi float64, eta float64) float64 {

	var out float64 = 0

	for _, ijn := range IJn2cPH {
		out += ijn[2] * math.Pow(pi+25, ijn[0]) * math.Pow(eta-1.8, ijn[1])
	}
	return out
}
 ...
 * @return
 */
func theta2cPS(pi float64, sigma float64) float64 {

	var out float64 = 0

	for _, ijn := range IJn2cPS {
		out += ijn[2] * math.Pow(pi, ijn[0]) * math.Pow(2-sigma, ijn[1])
	}
	return out
}
