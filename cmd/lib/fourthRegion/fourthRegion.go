...
func checkP(pressure float64) {
    ...
    func  densitiesRegion3(pressure float64,  enthalpies []float64) []float64 {
        return []float64{
            1 / thirdRegion.SpecificVolumePH(pressure, enthalpies[0]),
            1 / thirdRegion.SpecificVolumePH(pressure, enthalpies[1]),
			};
    }
...
//     @Override
   ...
   func SaturationPressureT (saturationTemperature float64) float64 {
        ...
        return math.Pow(2 * C / (-B + math.Sqrt(B * B - 4 * A * C)), 4) * pRef;
    }
    ...
    func  SaturationTemperatureP(saturationPressure float64)float64 {
        ...
        return (n9plusD - math.Sqrt(n9plusD * n9plusD - 4 * (n[8] + n[9] * D))) / 2 * Tref;
    }
...
//     @Override
    ...
    func SpecificEntropySaturatedLiquidP(pressure float64) float64 {
        ...
        if (pressure > ps13) {
            ...
            return thirdRegion.SpecificEntropyRhoT(1 / v, Ts);
        }
...
//     @Override
    ...
    func SpecificInternalEnergySaturatedLiquidP(pressure float64) float64 {
        ...
        if (pressure > ps13) {
            ...
            return thirdRegion.SpecificInternalEnergyRhoT(1 / v, Ts);
        }
...
//     @Override
    ...
    func SpecificVolumeSaturatedLiquidP( pressure float64)float64 {
        ...
        if (pressure > ps13) {
            if (pressure == constants.Pc) {
                return 1 / constants.Rhoc;
            }
           			v := []float64{firstRegion.SpecificVolumePT(ps13, constants.T13), 1 / constants.Rhoc, math.NaN(), math.NaN(), math.NaN()}
                    dp := []float64{thirdRegion.PressureRhoT(1 / v[0], Ts) - pressure, thirdRegion.PressureRhoT(constants.Rhoc, Ts) - pressure, math.NaN()};
            ...
            for i := 0; i < ITERATION_LIMIT; i++ {
                ...
                tol1 := 2 * math.SmallestNonzeroFloat64* math.Abs(v[1]) + TOLERANCE / 2
                        xm := (v[2] - v[1]) / 2;
                ...
                dp[1] = thirdRegion.PressureRhoT(1 / v[1], Ts) - pressure;
            }
            return v[1];
    ...
    func SpecificVolumeSaturatedVapourP(pressure float64) float64 {
        ...
        if (pressure > ps13) {
           ...
           v :=[]float64{math.NaN(), secondRegion.SpecificVolumePT(ps13, constants.T13), math.NaN(), math.NaN(), math.NaN()}
        	dp := []float64{math.NaN(), thirdRegion.PressureRhoT(1 / v[1], constants.T13) - pressure, math.NaN()};
            ...
            for i := 0; i < 1000; i++ {
                ...
                dp[0] = thirdRegion.PressureRhoT(1 / v[0], Ts) - pressure;
