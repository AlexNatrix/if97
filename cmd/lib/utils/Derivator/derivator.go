...
func partialDerivativePH(pressure float64, enthalpy float64, p_or_T quantity.Quantity, y quantity.Quantity, z quantity.Quantity) float64 {
	...
	var dy, dz float64
	h := []float64{
		fourthRegion.SpecificEnthalpySaturatedLiquidP(pressure),
		fourthRegion.SpecificEnthalpySaturatedVapourP(pressure),
	} // [kJ/kg]
	v := []float64{
		fourthRegion.SpecificVolumeSaturatedLiquidP(pressure),
		fourthRegion.SpecificVolumeSaturatedVapourP(pressure)} // [m³/kg]
...
func derivativePQ(pressure float64, pT quantity.Quantity, z quantity.Quantity) []float64 {
	...
	if ps13 <= pressure {
		...
		*/
		...
		rho := []float64{
			1.0 / thirdRegion.SpecificVolumePH(pressure, h[0]),
			1.0 / thirdRegion.SpecificVolumePH(pressure, h[1]),
		} // [kg/m³]
